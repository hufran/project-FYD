...
app.controller('myCtrl', function($scope, $http,$location){
    ...
    $scope.NoInquiry.push({name:"小区名称",value:"",point:false,pointMsg:"请填写小区名称！",pattern:/^.{0,50}$/u,patternErrorMsg:"小区名称输入文字数超限！",show:true,options:false});
    ...
    $scope.hasInquiry.push({name:"小区名称",value:"",point:false,pointMsg:"请填写小区名称！",pattern:/^.{0,50}$/u,patternErrorMsg:"小区名称输入文字数超限！",show:true,options:false});
    ...
    $scope.autoInquiry.push({name:"小区名称或房屋地址",value:"",point:false,pointMsg:"请填写小区名称！",pattern:/^.{0,50}$/u,patternErrorMsg:"小区名称输入文字数超限！",show:true,options:false});
    ...
    $scope.areaAddress = "";
    //$scope.alertMsg=false;
    let startTime,endTime,execTime= 3,timeRequest,timeUid,tempValue,count=0;
    ...
    $scope.$watch("roomInfo",function(){
        ...
        $scope.changeStatus=function(event){
            ...
            if(index==0){
                ...
                //其他项目的选择
                lastTimeStatus=$scope.dataList[index-1].options.filter((value,index)=>value.checked==true);
        ...
        $scope.changeValue=function(event){
            ...
            switch(attr){
                        ...
                        $scope.dataList.filter((value)=>{
                            if(value.type==type){
                                if(lastTimeStatus!=""){
                                    ...
                                    value.options.filter((obj)=>{obj.checked=false;})
                                }
                ...
                case "sure":
                    if(type){
                        //表示非房本选择项
                        $scope.dataList.filter((value,index)=>{
                            if(value.type==type){console.log("value:",value);
                                let valueList=value.options.filter((data,i)=>data.checked==true);
                        ...
                        //表示房本选择项
                        $scope.class={showSelect:false,hideSelect:true};
                        $scope.roomInfo.filter((value,index)=>{
                        ...
                        $scope.dataList.forEach((value)=>value.value="");
                    }
                    break;
                default:
                    break;
    ...
    $scope.$watch('searchValue+areaNameResult',function(){
        if($scope.searchValue.length>0&&tempValue!=$scope.searchValue){
            var util={
                ...
                createServer:function (url,data,success,error,timeout,method){
                    let isError=false,timeuid,self=this;
            ...
            if($scope.cacheTime.levelTwo>execTime){console.log("这里是level2级请求");
                $scope.alertMsg=true;
            ...
            if($scope.cacheTime.levelThree>execTime){console.log("这里是level3级请求");
                $scope.alertMsg=true;
                if(timeUid){clearTimeout(timeUid);}
                timeUid=setTimeout(function(){
                    timeUid=null;
    ...
    $scope.changeOptions=function(type,index,event){
        ...
        if($scope.autoPage){
            $scope.dataList.filter((value)=>{
                if(value.type==type){
                value.options.forEach((data)=>data.checked=false);
            ...
            if(type==0){
                //有无房本操作
                $scope.roomInfo.forEach((value)=>value.checked=false);
    ...
    $scope.validate=function(index){
        let length=$scope.dataList.length;
        if(index<length&&index>=0){
            if($scope.dataList[index].value==null||$scope.dataList[index].value.length<1){
                ...
                if($scope.dataList[index].pattern){
                    let pattern=$scope.dataList[index].pattern,value=$scope.dataList[index].value;
            ...
            if(i!=length-1){
                if($scope.dataList[i].value==null||$scope.dataList[i].value.length<1){
                    ...
                    if($scope.dataList[i].pattern){
                        let pattern=$scope.dataList[i].pattern,value=$scope.dataList[i].value;
                        let bol=pattern.test(value);
                        if(!bol){
    ...
    $scope.submit=function(){
        ...
        let [{name:nameValue}]=$scope.roomInfo.filter((value)=>value.checked);
        let param={};//需要上传的数据
        if(nameValue==="有房本"){
            $scope.uploadErrorMsg="图片必须上传，且图片大小不得超过2M！";
            if(checkWechat()&&false){
            ...
            ([{value:param.areaName}, {value:param.housingMortgage},{value:param.mortgageAmount}]=$scope.dataList);

        }else{
            ([{value:param.roomAddress},{value:param.areaName},{value:param.housingArea},{name:param.housingType},{value:param.housingMortgage},{value:param.mortgageAmount}]=$scope.dataList);
        }
...
console.log(param);
        ...
        let startUpload=function(formData){
            $scope.alertMsg=true;
            $.ajax({
                url: './userCenter/save?openId='+$scope.inquiryData.openId,
                type: 'POST',
